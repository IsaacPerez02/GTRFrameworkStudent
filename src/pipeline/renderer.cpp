...
Renderer::Renderer(const char* shader_atlas_filename)
{
	...
	lighting_FBO.create(1024, 768,
		...
		true);
	// SSAO FBO (solo un canal, puedes hacerlo a mitad de resoluci�n si quieres)
	ssao_FBO.create(
...
void Renderer::parseNodes(SCN::Node* node, Camera* cam) {
	...
	if (node->mesh) {
		...
		//	return; // est� fuera del frustum, no lo renderizamos
		}
...
void Renderer::renderScene(SCN::Scene* scene, Camera* camera)
{
	...
	if (use_ssao) {
		...
		// opcional: un blur o upsample aqu� si implementas 2.6
		return;
	}
	...
	if (use_deferred_rendering) {
		...
		if (black_hole_2d_or_3d) {
			...
			GFX::Mesh blackhole = GFX::Mesh(); //dont use it, it is more for understanding the parameters better
...
void Renderer::showUI()
{
	...
	if (use_ssao) {
		...
		// Si cambi� el n�mero de muestras o el modo, regeneramos el kernel
		if (ssao_sample_count != prev_samples || use_ssao_plus != prev_plus) {
			ssao_kernel = generateSpherePoints(
				...
				use_ssao_plus           // hemisferio si �+� est� activo
			);
...
void Renderer::renderRing(const Matrix44 model, GFX::Mesh* mesh, SCN::Material* material)
{
	...
	glEnable(GL_DEPTH_TEST);
	glDepthMask(GL_TRUE);
	glEnable(GL_BLEND);  // <- Activa el blending aquí
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  // <- Define la función de mezcla para transparencia

	mesh->render(GL_TRIANGLES);

	glDisable(GL_BLEND);  // Opcional: desactiva el blending después de dibujar si no lo necesitas para otros objetos
...
// Renderer.cpp (m�todo adicional claramente separado para post-procesado)
